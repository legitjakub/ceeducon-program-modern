...
cp -R "$ROOT/wordpress-plugin/ceeducon-control-center" "$STAGE/ceeducon-control-center"
...
rm -f \
  ...
  "$DIST/ceeducon-control-center.zip" \
...
(cd "$STAGE" && zip -X -q -r "$DIST/ceeducon-control-center.zip" ceeducon-control-center)
